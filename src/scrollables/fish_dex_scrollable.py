...
class FishDexScrollable(AbstractScrollable):
    ...
    # Takes in a list of tuples[entry, caught?]
    @staticmethod
    async def create(fish_dex: list[tuple[FishEntry, bool]]) -> 'FishDexScrollable':
    ...
    async def output(self) -> str:
        ...
        strings = []
        for fish_entry, caught in fishes:
            if not isinstance(fish_entry, FishEntry):
                string = "ERROR"
            elif caught:
                string = f"**{fish_entry.rarity.name[0]}** | {fish_entry.get_emoji()} **`{fish_entry.name}`**"
            else:
                string = f"**{fish_entry.rarity.name[0]}** | **`?????`**"
